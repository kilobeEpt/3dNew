...
if (strpos($requestUri, '/api') === 0) {
    // Change to project root so API can access bootstrap and other files
    chdir($projectRoot);
    require $projectRoot . '/api/index.php';
    exit;
}
...
if (strpos($requestUri, '/admin') === 0) {
    // Change to project root so admin can access bootstrap and other files
    chdir($projectRoot);
...
// Build file path
$filePath = $publicRoot . $requestUri;

// Security check: prevent directory traversal attacks
$realPath = realpath($filePath);
$realPublicRoot = realpath($publicRoot);

if ($realPath === false || strpos($realPath, $realPublicRoot) !== 0) {
    // File doesn't exist or is outside public root
    serveNotFound();
    exit;
}
...
if (is_file($realPath)) {
    // Security: Never serve PHP files as static content
...
function serveNotFound(): void
{
    ...
    $notFoundPage = $publicRoot . '/404.html';
